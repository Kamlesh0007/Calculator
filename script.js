...
const negtate = document.querySelector("[data-negate]");
...
toggleBtn.forEach(btn => {
    btn.addEventListener("click", () => {
        themeChange(btn.value); // returning click button value
    });
})
...
negtate.addEventListener("click",()=>{
    negate();
    displayNum();
})
...
function negate(){
    if (currentOperand === '' || currentOperand === '.' || currentOperand === '0') return;
    currentOperand = currentOperand - (currentOperand * 2)
  }